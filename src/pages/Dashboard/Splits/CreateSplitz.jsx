      ...
      <main className="max-w-3xl mx-auto space-y-4 md:space-y-6">
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <button
            ...
            className="flex items-center gap-1.5 md:gap-2 text-gray-600 hover:text-gray-900 font-medium text-xs md:text-sm"
          >
            <ChevronLeft size={16} className="md:w-[18px] md:h-[18px]" /> <span>Back to Splits</span>
          </button>
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 text-center flex-1">
            Create New Split
          </h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          ...
          <section className="p-4 md:p-6 rounded-xl shadow-sm bg-white space-y-4 md:space-y-6">
            ...
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Split Title *</label>
              <input
                type="text"
                placeholder="e.g. Shared Costco Groceries"
                className={`w-full px-3 md:px-4 py-2 md:py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition text-sm ${
            ...
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Split Type/Category *</label>
              <select
                className={`w-full px-3 md:px-4 py-2 md:py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-sm ${
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-sm"
                rows="2"
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 md:p-6 hover:border-green-500 transition cursor-pointer">
                <label className="flex flex-col items-center gap-3 md:gap-4 cursor-pointer">
                  ...
                  <div className="text-center">
                    ...
                    />
                    <div>
                      <span className="text-green-600 font-medium text-sm">Choose File</span>
              ...
              {imagePreview && (
                <div className="mt-2 md:mt-3">
                  ...
                  <button
                    ...
                    className="mt-1.5 md:mt-2 text-xs text-red-600 hover:text-red-800"
                  >
                    Remove image
                  </button>
                </div>
          ...
          <section className="p-4 md:p-6 rounded-xl shadow-sm bg-white space-y-4 md:space-y-6">
            <div className="mb-3 md:mb-4">
              <h2 className="text-base md:text-lg font-semibold text-gray-900">Cost and Participants</h2>
            </div>
            
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Total Amount (₦) *</label>
              <input
                type="number"
                min="1"
                step="0.01"
            ...
            <div className="space-y-2 md:space-y-3">
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Split Method *</label>
              {splitMethods.map((method) => (
                <button
                  key={method.value}
                  type="button"
                  onClick={() => setValue('split_method', method.value)}
                  className={`w-full p-3 md:p-4 border-2 rounded-lg text-left transition ${
                ...
                >
                  ...
                  <p className="text-[10px] md:text-xs text-gray-600 ml-6 md:ml-8">{method.description}</p>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-2 md:gap-3 p-3 md:p-4 bg-green-50 rounded-lg">
              <input
                ...
                className="w-4 h-4 md:w-5 md:h-5 accent-green-600 rounded cursor-pointer"
                id="include-me"
              />
              <label htmlFor="include-me" className="text-xs md:text-sm font-medium text-gray-700 cursor-pointer">
          ...
          <section className="p-4 md:p-6 rounded-xl shadow-sm bg-white space-y-4 md:space-y-6">
            <div className="mb-3 md:mb-4">
              <h2 className="text-base md:text-lg font-semibold text-gray-900 flex items-center gap-2">
                <Calendar size={18} className="md:w-5 md:h-5" /> Start & End Period
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">End Date *</label>
                <input
                  type="date"
                  className={`w-full px-3 md:px-4 py-2 md:py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-sm ${
          ...
          <section className="p-4 md:p-6 rounded-xl shadow-sm bg-white space-y-4 md:space-y-6">
            <div className="mb-3 md:mb-4">
              <h2 className="text-base md:text-lg font-semibold text-gray-900">Location and Visibility</h2>
            </div>
            
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Split Location *</label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                max="10"
                step="1"
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-green-600 md:[&::-webkit-slider-thumb]:h-5 md:[&::-webkit-slider-thumb]:w-5"
            ...
            <div>
              <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1.5 md:mb-2">Rules (Optional)</label>
              <textarea
          ...
          <section className="p-4 md:p-6 rounded-xl shadow-sm bg-white space-y-4 md:space-y-6">
            <div className="mb-3 md:mb-4">
              <h2 className="text-base md:text-lg font-semibold text-gray-900">Safety Information</h2>
            </div>
            
            <div className="p-3 md:p-4 border-l-4 border-red-500 bg-red-50 rounded flex items-start gap-2.5 md:gap-3">
              <AlertCircle size={16} className="text-red-600 mt-0.5 flex-shrink-0 md:w-5 md:h-5" />
              <div>
                <h4 className="font-semibold text-red-700 text-xs md:text-sm mb-0.5 md:mb-1">Mandatory Refund Rule</h4>
                <p className="text-[10px] md:text-xs text-red-600">A 5% charge applies if the split is canceled before 70% participation.</p>
              </div>
            </div>
            
            <div className="p-3 md:p-4 border-l-4 border-green-600 bg-green-50 rounded flex items-start gap-2.5 md:gap-3">
              <Shield size={16} className="text-green-600 mt-0.5 flex-shrink-0 md:w-5 md:h-5" />
              <div>
                <h4 className="font-semibold text-green-700 text-xs md:text-sm mb-0.5 md:mb-1">Secure Payment Protection</h4>
                <p className="text-[10px] md:text-xs text-green-600">All payments are protected and released only when conditions are met.</p>
              </div>
            </div>
          </section>
