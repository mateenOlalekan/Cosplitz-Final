// src/pages/dashboard/SplitzDetail.jsx
import {
  ChevronLeft,
  Share2,
  Users,
...
function SplitzDetail() {
  ...
  const { data: split, isLoading, error } = useSplit(id);
  ...
  // Handle join - navigate to payment page
  const handleJoinAndPay = () => {
  ...
  const isCreator = () => {
    ...
    return split.user === parseInt(currentUserId) || split.role === 'creator';
  };

  const hasJoined = () => {
    if (!split?.participants) return false;
    const currentUserId = parseInt(localStorage.getItem('userId'));
    return split.participants.some(p => p.id === currentUserId || p.user_id === currentUserId);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <div className="text-xl font-semibold text-gray-700 mb-2">
          {error ? 'Error loading split' : 'Split not found'}
        </div>
        <button
          onClick={() => navigate(-1)}
  ...
  const userHasJoined = hasJoined();

  return (
    <div className="flex flex-col w-full min-h-screen bg-white">
      {/* Header */}
      <header className="flex sticky top-0 items-center justify-between px-4 md:px-10 py-4 shadow-sm bg-white z-10">
      ...
      {/* Content */}
      <div className="flex flex-col w-full mx-auto max-w-5xl px-4 md:px-10 pt-6 gap-6 pb-24">
        {/* Title & Tags */}
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            {userIsCreator && (
              <span className="px-4 py-1 text-sm bg-purple-100 text-purple-800 rounded-full">Creator</span>
            )}
            {userHasJoined && !userIsCreator && (
              <span className="px-4 py-1 text-sm bg-blue-100 text-blue-800 rounded-full">Joined</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white p-5 shadow-sm shadow-[#D0C9D6]">
          ...
          <div className="flex justify-between items-start pb-2 border-b border-[#D0C9D6] pt-3">
            <div className="flex flex-col gap-4 text-sm text-gray-600">
              <div className="flex items-center gap-2"><Users size={15} /><span>Participants</span></div>
              <div className="flex items-center gap-2"><Clock size={15} /><span>Deadline</span></div>
              <div className="flex items-center gap-2"><MapPin size={15} /><span>Distance</span></div>
            </div>
          ...
          <div className="flex flex-col gap-2 pt-3">
            <div className="w-full h-2 bg-[#D0C9D6] rounded-full overflow-hidden">
              <div className="h-full bg-[#1F8225] rounded-full transition-all" style={{ width: `${progress}%` }} />
            </div>
        ...
        <div className="rounded-xl bg-white p-5 shadow-sm shadow-[#D0C9D6]">
          ...
          {split.start_date && (
            <p className="text-[#1F8225] mt-1">Starts: {new Date(split.start_date).toLocaleDateString()}</p>
          )}
        </div>
        ...
        <div className="rounded-xl bg-white p-5 shadow-sm shadow-[#D0C9D6]">
          ...
          <p className="text-[#67707E] mt-2">{split.description || "No description provided."}</p>
          {split.rules && (
            <div className="mt-4">
          ...
          <div className="rounded-xl bg-white p-5 shadow-sm shadow-[#D0C9D6]">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-gray-500">
                {split.participants.filter((p) => p.has_paid).length} paid, {" "}
                {participants - split.participants.filter((p) => p.has_paid).length} pending
              </span>
            </div>
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-blue-200 rounded-full flex items-center justify-center text-blue-700 text-sm font-bold">
                      {participant.name?.charAt(0) || participant.first_name?.charAt(0) || "U"}
                    </div>
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-gray-900">
                          {participant.name || `${participant.first_name || ''} ${participant.last_name || ''}`.trim() || `User ${index + 1}`}
                        </span>
                  ...
                  <span className={`px-3 py-1 text-xs rounded-full font-medium ${
                    participant.has_paid ? "bg-emerald-500 text-white" : "bg-gray-200 text-gray-700"
                  }`}>
        ...
        {/* Action Buttons */}
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 md:relative md:border-0 md:p-0 md:bg-transparent flex gap-3 z-20">
          ...
          {userIsCreator ? (
            <button disabled className="flex-1 bg-gray-400 text-white font-semibold py-3 rounded-xl cursor-not-allowed">
              You are the Creator
            </button>
          ) : userHasJoined ? (
            <button disabled className="flex-1 bg-blue-500 text-white font-semibold py-3 rounded-xl cursor-not-allowed">
              Already Joined
            </button>
          ) : (
            <button
            ...
            >
              {isFull ? "Split is Full" : `Join & Pay ${formatCurrency(pricePerPerson)}`}
            </button>
          )}
        </div>
      </div>
