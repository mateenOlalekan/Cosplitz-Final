...
// Step 1 → Step 2 (after register)
  ...
  const handleVerifyOTP = async (otp) => {
    const res = await verifyOTP(null, otp); // userId auto-injected from tempRegister
    if (!res.success) {
      // Error is already set in store
      return false;
    }
  ...
  const handleResendOTP = async () => {
    await resendOTP(); // Uses tempRegister.userId automatically
  };
