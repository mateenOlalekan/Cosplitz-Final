...
export default function Register() {
  ...
  useEffect(() => {
    ...
    // Check if this is an existing user (not in registration flow) with completed onboarding
    const isExistingUser = user && 
                          !tempRegister && 
                          !justRegistered && 
                          onboardingComplete === true;
    
    console.log('Register - User status:', {
      hasUser: !!user,
      hasTempRegister: !!tempRegister,
      justRegistered,
      onboardingComplete,
      currentStep,
      isExistingUser
    ...
    // Only redirect if it's an existing user who has completed everything
    if (isExistingUser && currentStep === 1) {
      console.log('Existing user with completed onboarding, redirecting to dashboard');
      navigate('/dashboard', { replace: true });
    }
  }, [user, isUserLoading, tempRegister, justRegistered, onboardingComplete, currentStep, navigate]);

  // Restore step if user has temp registration (they started registering)
  useEffect(() => {
    if (tempRegister && currentStep === 1) {
      console.log('Restoring registration step from tempRegister');
      setCurrentStep(2);
    }
  ...
  const handleVerifyOTP = async (otp) => {
    ...
    try {
      ...
      // User must click "Continue Setup" button to proceed to onboarding
      setCurrentStep(3);
  ...
  const handleBackToRegistration = () => {
    console.log('Going back to registration, clearing all data');
